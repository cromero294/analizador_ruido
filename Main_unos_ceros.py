...
try:
    ...
    for particion in particiones:
        ...
        clf1, clf2 = cambiaClase.entrenamiento_unos_ceros(datostrain, tree.DecisionTreeClassifier, 100)
        score_final += clf1.score(datostest[:-1], np.ones(datostest.shape[0]))
        score_final += clf2.score(datostest[:-1], np.ones(datostest.shape[0]))
